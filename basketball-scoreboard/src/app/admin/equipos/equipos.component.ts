...
export class EquiposComponent implements OnInit {
  ...
  saveEquipo() {
    if (this.editingEquipo) {
      ...
      this.http.put<Equipo>(`http://104.131.96.162:5000/api/equipos/${this.currentEquipo.id}`, updateData).subscribe({
        next: () => {
          this.loadEquipos();
          this.cancelEdit();
        },
      ...
      this.http.post<Equipo>('http://104.131.96.162:5000/api/equipos', createData).subscribe({
        next: () => {
          this.loadEquipos();
          this.cancelEdit();
        },
  ...
  deleteEquipo(id: number) {
    if (confirm('¿Estás seguro de eliminar este equipo?')) {
      this.http.delete(`http://104.131.96.162:5000/api/equipos/${id}`).subscribe({

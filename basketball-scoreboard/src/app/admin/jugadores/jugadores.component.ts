...
export class JugadoresComponent implements OnInit {
  ...
  loadJugadores() {
    this.http.get<Jugador[]>('http://104.131.96.162:5000/api/jugadores').subscribe({
      next: (jugadores) => {
  ...
  saveJugador() {
    ...
    if (this.editingJugador) {
      ...
      this.http.put<Jugador>(`http://104.131.96.162:5000/api/jugadores/${this.currentJugador.id}`, updateData).subscribe({
        next: () => {
          this.loadJugadores();
          this.cancelEdit();
        },
      ...
      this.http.post<Jugador>('http://104.131.96.162:5000/api/jugadores', createData).subscribe({
        next: () => {
          this.loadJugadores();
          this.cancelEdit();
        },
  ...
  deleteJugador(id: number) {
    if (confirm('¿Estás seguro de eliminar este jugador?')) {
      this.http.delete(`http://104.131.96.162:5000/api/jugadores/${id}`).subscribe({
